...
esp_lcd_panel_io_handle_t create_lcd_io()
{
    ...
    ESP_ERROR_CHECK(spi_bus_initialize(
        BOARD_LCD_SPI_HOST, &bus_config, BOARD_LCD_SPI_DMA_CHANNEL));
...
esp_lcd_panel_handle_t create_lcd_panel(esp_lcd_panel_io_handle_t io)
{
    ...
    ESP_ERROR_CHECK(esp_lcd_panel_mirror(
        panel, BOARD_LCD_NATIVE_MIRROR_X, BOARD_LCD_NATIVE_MIRROR_Y));
...
esp_lcd_touch_handle_t create_touch()
{
    ...
    // Field assignment avoids a designator-order issue in the component's C macro.
    ...
    io_config.flags.disable_control_phase = BOARD_TOUCH_I2C_DISABLE_CONTROL_PHASE;
...
void set_backlight(bool enabled)
{
    ...
    ESP_ERROR_CHECK(gpio_set_level(
        ...
        enabled ? BOARD_LCD_BACKLIGHT_ON_LEVEL : !BOARD_LCD_BACKLIGHT_ON_LEVEL));
}
